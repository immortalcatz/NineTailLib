...
import static keri.ninetaillib.lib.render.ctm.BlockRendererCTM.SubSide.*;
import static keri.ninetaillib.lib.render.ctm.BlockRendererCTM.Vert.*;
...
public class BlockRendererCTM extends BlockRenderer {
    ...
    protected LegacyTessellator tessellator; /** = Tessellator.instance;*/
    ...
    public BlockRendererCTM(VertexBuffer buffer){
        super(buffer);
    ...
    public boolean renderStandardBlock(Block block, int x, int y, int z) {
        ...
        by = y;
        bz = z;
        meta = BlockAccessUtils.getBlockMetadata(blockAccess, new BlockPos(x, y, z));
    ...
    */
    ...
    protected void pre(EnumFacing face) {
        manager.preRenderSide(this, blockAccess, bx, by, bz, face);
    }

    protected void post(EnumFacing face) {
    ...
    protected enum SubSide {
        ...
        private EnumFacing calcNormal() {
            ...
            if (xTot % 4 == 0) {
                return xTot > 0 ? EnumFacing.EAST : EnumFacing.WEST;
            } else if (yTot % 4 == 0) {
                return yTot > 0 ? EnumFacing.UP : EnumFacing.DOWN;
            } else if (zTot % 4 == 0) {
                return zTot > 0 ? EnumFacing.SOUTH : EnumFacing.NORTH;
            }

            /**
             * THIS IS NOT RIGHT, MAKE IT CHECK FOR NULL INSTEAD!!!
             */
            return EnumFacing.SOUTH;
        }

        void render(BlockRendererCTM inst) {
...
public enum Vert {
        ...
        void render(BlockRendererCTM inst, EnumFacing normal, int cacheID) {
            //if (inst.enableAO) {
